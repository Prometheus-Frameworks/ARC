...
def test_build_baselines_cli_attempts_both_parquet_exports(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()
    ...
    parquet_calls: list[Path] = []

    def _capture_export_parquet(df: pd.DataFrame, path: str | Path) -> None:
        _ = df
        parquet_calls.append(Path(path))

    monkeypatch.setattr("arc.cli.export_parquet", _capture_export_parquet)
    ...
    assert result.exit_code == 0
    assert out_dir / "arc_cohort_baselines.parquet" in parquet_calls
    assert out_dir / "arc_career_year_baselines.parquet" in parquet_calls
